...
def db_get_top_users_by_messages():
    data = db_select("SELECT n.nick, COUNT(l.message), l.name AS count FROM cardboardlog l, cardboardnick n WHERE l.name = n.nick GROUP BY l.name ORDER BY count DESC LIMIT 5")
    return data

def db_get_top_users_by_links():
    data = db_select("SELECT n.nick, COUNT(l.url) AS count, l.name FROM cardboardlinks l, cardboardnick n WHERE l.name = n.nick GROUP BY l.name ORDER BY count DESC LIMIT 5")
    return data
...
def db_get_top_users_by_message_link_ratio():
    data = db_select("SELECT n.nick, ((SELECT COUNT(url) FROM cardboardlinks WHERE cardboardlinks.name = l.name) / CAST(COUNT(message) AS REAL)) * 100 AS ratio, l.name FROM cardboardlog l, cardboardnick n WHERE l.name = n.nick AND l.name IN (SELECT DISTINCT name FROM cardboardlinks) GROUP BY l.name ORDER BY ratio DESC LIMIT 5")
    return data
